...
class DashboardCard(QFrame):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            ...
            "background: rgba(28, 31, 39, 235); border: 1px solid rgba(255,255,255,24);"
            "border-radius: 18px;}"
        )
...
class QuickAction(QPushButton):
    def __init__(self, title: str, icon_path: str, parent=None):
        ...
        self.setIconSize(self.iconSize().expandedTo(self.sizeHint() / 8))
        self.setStyleSheet(
            "QPushButton {background: rgba(255,255,255,10); border: 1px solid rgba(255,255,255,22);"
            "border-radius: 12px; padding: 0 16px; text-align: left; font-weight: 600;}"
            "QPushButton:hover {background: rgba(245,158,11,28); border-color: rgba(245,158,11,100);}"
...
class HomePage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        eyebrow.setStyleSheet("color: #F59E0B; font-size: 11px; font-weight: 800; letter-spacing: 2px;")
        ...
        self.continue_button.setStyleSheet(
            "QPushButton {background: #F59E0B; color: #17191F; border: 0; border-radius: 11px;"
            "padding: 0 20px; font-weight: 800;} QPushButton:hover {background: #FBBF24;}"
            "QPushButton:disabled {background: rgba(255,255,255,20); color: rgba(255,255,255,80);}"
        )
    ...
    def refresh(self) -> None:
        ...
        if exists:
            try:
                count = sum(1 for item in path.iterdir() if item.is_file() and item.suffix.lower() in IMAGE_EXTENSIONS)
            except OSError:
                count = 0
        ...
        self.project_meta.setText(f"{count} фотографий")
        ...
        self.templates_stat.value_label.setText(str(stats.get("templates_created", 0)))
        last_run = stats.get("last_run", "")
        if last_run:
            try:
                last_run = datetime.fromisoformat(last_run).strftime("%d.%m.%Y")
            except ValueError:
                pass
